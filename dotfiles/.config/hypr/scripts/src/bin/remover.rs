...
#[command(
    name = "remover",
    about = "Remove packages using Cargo, pnpm, pacman, or an AUR helper"
)]
struct Cli {
...
fn main() {
        ...
        && packages.aur.is_empty()
    {
        ...
        println!();
        println!("Usage:");
        println!("  remover --cargo <package>");
        println!("  remover --pnpm <package>");
        println!("  remover --pacman <package>");
        println!("  remover --aur <package>");
    ...
    failed |= remove("cargo", &packages.cargo, |package| {
        ("cargo", vec!["uninstall".to_owned(), package.to_owned()])
    });
    failed |= remove("pnpm", &packages.pnpm, |package| {
        (
            "pnpm",
            vec!["remove".to_owned(), "--global".to_owned(), package.to_owned()],
        )
    });
    failed |= remove_with_output(
        "pacman",
        &packages.pacman,
        |package| {
            (
    ...
    failed |= remove_aur(&packages.aur);

    if failed {
        println!("\nOne or more packages could not be removed.");
    } else {
...
fn remove<F>(manager: &str, packages: &[String], command_for: F) -> bool
where
    F: Fn(&str) -> (&str, Vec<String>),
{
    remove_with_output(manager, packages, command_for, false)
}

fn remove_with_output<F>(
    manager: &str,
    packages: &[String],
    command_for: F,
    interactive: bool,
    ...
    F: Fn(&str) -> (&str, Vec<String>),
{
    ...
    for package in packages {
        ...
        if !run_command(program, &arguments, interactive) {
            eprintln!("Failed to remove {package} with {manager}.");
            failed = true;
        }
    }
    failed
...
fn remove_aur(packages: &[String]) -> bool {
    let Some(manager) = ["yay", "paru"].iter().find(|manager| which(*manager)) else {
        if !packages.is_empty() {
            eprintln!("AUR removal requires yay or paru.");
        }
