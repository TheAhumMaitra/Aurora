...
fn apply_rofi_config(name: &str, src_path: &PathBuf) {
    let home = dirs::home_dir().expect("Could not get HOME directory");
    let destination = home.join(".config").join("rofi").join("config.rasi");

    match fs::copy(src_path, destination) {
        Ok(_) => {
            println!("Rofi config `{name}` applied");
            let _ = Command::new("notify-send")
                .arg(format!("{name} rofi config is applied!"))
                .spawn();
        }
        Err(err) => {
            eprintln!("Failed to apply rofi config `{name}`: {err}");
            let _ = Command::new("notify-send")
                .arg(format!("Failed to apply {name} rofi config!"))
                .spawn();
        }
    }
}
...
fn build_ui(app: &Application) {
    ...
    list_box.connect_row_activated(move |_, row| {
        ...
        if let Some(path) = index.get(name.as_str()) {
            apply_rofi_config(name.as_str(), path);
        }
    });
...
fn main() {
    let app = Application::builder()
        .application_id("com.aurora.rofi_config_switcher")
        .build();
