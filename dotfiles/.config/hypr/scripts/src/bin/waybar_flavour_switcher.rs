...
fn copy_waybar_files(src_dir: &PathBuf, dst_dir: &PathBuf) {
    let files = ["config.jsonc", "style.css"];
...
fn apply_waybar_config(name: &str, src_dir: &PathBuf) {
    let home = home_dir().expect("Could not get HOME directory");
    let destination = home.join(".config").join("waybar");

    copy_waybar_files(src_dir, &destination);

    let _ = Command::new("waybar_refresh").spawn();

    println!("Waybar config `{name}` applied");
    let _ = Command::new("notify-send")
        .arg(format!("{name} waybar config is applied!"))
        .spawn();
}
...
fn build_ui(app: &Application) {
    ...
    list_box.connect_row_activated(move |_, row| {
        ...
        if let Some(path) = index.get(name.as_str()) {
            apply_waybar_config(name.as_str(), path);
        }
    });
...
fn main() {
    let app = Application::builder()
        .application_id("com.aurora.waybar_config_switcher")
        .build();
