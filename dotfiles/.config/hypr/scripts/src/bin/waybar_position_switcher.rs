...
use aurora::load_css;
use aurora::waybar_position_change;
use gtk4::prelude::*;
use gtk4::{
...
fn run_position_change(position: &str) {
    match waybar_position_change(position.to_string()) {
        Ok(_) => {
            let _ = Command::new("waybar_refresh").spawn();
            println!("Waybar position `{position}` applied");
            let _ = Command::new("notify-send")
                .arg(format!("Waybar position set to {}!", position))
                .spawn();
        }
        Err(err) => {
            eprintln!("Failed to change Waybar position to `{position}`: {err}");
            let _ = Command::new("notify-send")
                .arg(format!("Failed to set Waybar position to {}!", position))
                .spawn();
        }
    }
}
...
fn build_ui(app: &Application) {
    let window = ApplicationWindow::builder()
        .application(app)
        .title("Waybar Position Switcher")
        .default_width(340)
        .default_height(240)
        .decorated(false)
        .build();
    ...
    for position in POSITIONS {
        ...
        let label = Label::new(Some(position));
    ...
    list_box.connect_row_activated(move |_, row| {
        ...
        let position = label.text();

        if POSITIONS.contains(&position.as_str()) {
            run_position_change(&position);
        }
    });
...
fn main() {
    let app = Application::builder()
        .application_id("com.aurora.waybar_position_switcher")
        .build();
